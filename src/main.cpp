...
#if defined(NDEBUG)
# error "Mahacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3bfa2a0c80d0a0d7d4c20cad152603aec6986faba5259f9ff87b604109e37374");
...
const string strMessageMagic = "Mahacoin Signed Message:\n";
...
{
    // Mahacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Mahacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Mahacoin: 1 days
static const int64 nTargetSpacing = 2 * 60; // Mahacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mahacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mahacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Mahacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "On August 25, 2014 - Amazon Acquires Twitch in $1 Billion Deal";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409859459;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 386854781;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Mahacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MahacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MahacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MahacoinMiner(CWallet *pwallet)
{
    printf("MahacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mahacoin-miner");
    ...
    try { loop {
        ...
        printf("Running MahacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MahacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MahacoinMiner, pwallet));
}
